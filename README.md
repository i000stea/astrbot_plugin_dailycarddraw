...
- 抽卡成功后会返回结果图片，单抽为单卡图，十连为横向长图；插件只做「命令接入 + 参数解析 + 权限判断 + 文本/图片展示」，抽卡概率、次数扣减、落库统计全部由云端后端 API 负责
...
| --- | --- |
| `/抽卡`、`/寻访` | 对默认卡池单抽 |
| `/抽卡 十连`、`/十连寻访` | 对默认卡池十连（`10连`、`ten` 等效） |
| `/抽卡 <卡池Key>`、`/寻访 <卡池Key>` | 对指定卡池单抽，例如 `/寻访 normal_pool` |
| `/抽卡 <卡池Key> 十连`、`/十连寻访 <卡池Key>` | 对指定卡池十连 |
| `/今日抽卡 [卡池Key]`、`/今日历史 [卡池Key]` | 查询本人今日次数与最近结果，省略卡池则用默认卡池 |
| `/抽卡历史 [页码] [每页数量]`、`/寻访历史 [页码] [每页数量]` | 分页查询本人历史记录，默认 `1 10` |
| `/抽卡统计`、`/寻访统计` | 查询本人累计统计（总抽数、累计积分、SSR / UR 数量等） |
| `/抽卡帮助`、`/寻访帮助` | 查看帮助文本（别名：`/抽卡help`、`/carddraw_help`） |
...
| 参数 | 取值 | 位置 |
| --- | --- | --- |
| 卡池 Key | 后端卡池表里的 `pool_key`，默认卡池由配置项 `default_pool_key` 决定（默认 `normal_pool`） | `/抽卡`、`/寻访`、`/十连寻访`、`/今日抽卡`、`/今日历史` 的第 1 个位置参数 |
| 抽取模式 | 留空 = 单抽；`十连` / `10连` / `ten` = 十连 | `/抽卡` 的最后一个位置参数；`/十连寻访` 固定为十连 |
| 页码 / 每页数量 | 整数，默认 `1` / `10`，服务端上限由 `MAX_PAGE_SIZE` 控制 | `/抽卡历史`、`/寻访历史` 的第 1、2 个位置参数 |
...
> 当前后端会返回 `image_url`：单抽生成单卡图，十连生成横向长图；插件会先发送图片，再发送上面的简短文字。若图片生成失败，则自动退化为完整文字结果。
